...
export default function Header() {
  ...
  const renderThemeChanger = () => {
    ...
    if (currentTheme === "dark") {
      return (
        <SunIcon
          className='lg:ml-4 w-8 h-8 bg-slate-500 rounded-md text-yellow-500'
          role='button'
      ...
      return (
        <MoonIcon
          className='lg:ml-4 w-8 h-8 bg-slate-500 rounded-md text-gray-900'
          role='button'
      ...
      <header className='flex navbar-fixed top-0 left-0 w-full items-center z-10 dark:bg-opacity-70 dark:bg-black bg-opacity-70'>
        <div className='container mx-auto max-w-screen-lg'>
          <div className='flex justify-between relative'>
            <div className='px-4'>
              <Link href='/'>
                <a className='font-bold text-2xl block py-6 relative'>
                  wrseno{" "}
                  <span className='text-gradient md:text-5xl text-3xl absolute lg:top-3.5 top-6'>
                    ⪤
                  </span>
                </a>
              </Link>
